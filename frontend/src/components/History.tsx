...
const parseHistoryDate = (value: string | null | undefined) => {
  ...
  const sqliteDateMatch = normalized.match(
...
export const History: React.FC<HistoryProps> = ({ userId }) => {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      let content =
        '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Reporte de Historial de Residuos</title>';
      content += "<style>";
      content +=
      ...
      const co2Saved = (
        entries.reduce((sum, e) => sum + e.weight, 0) * 0.18
      ).toFixed(2);
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-outline-variant">
                  ...
                  <th className="text-right font-label-sm text-on-surface-variant px-md py-md">
                    Peso (kg)
                  </th>
                  <th className="text-right font-label-sm text-on-surface-variant px-md py-md">
                    Fecha y Hora
            ...
            <div className="bg-surface-container-low p-md rounded-lg border border-outline-variant/30">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="font-display-lg text-tertiary-fixed-dim">
                    {co2Saved.toFixed(2)}
                  </p>
                  <p className="font-label-sm text-on-surface-variant">kg</p>
                </div>
