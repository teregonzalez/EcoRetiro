    ...
    >
      ...
      <section className="mb-lg grid grid-cols-1 gap-gutter md:grid-cols-3">
        <article className="rounded-xl border border-outline-variant bg-surface-container-lowest p-md shadow-sm">
          <p className="font-label-sm uppercase tracking-wider text-on-surface-variant">Usuarios Totales</p>
          <h3 className="mt-2 text-[36px] font-bold text-on-surface">{data?.metrics.totalUsers ?? 0}</h3>
          <p className="text-[12px] text-outline">Empresas activas en plataforma</p>
        </article>
        <article className="rounded-xl border border-outline-variant bg-surface-container-lowest p-md shadow-sm">
          <p className="font-label-sm uppercase tracking-wider text-on-surface-variant">Solicitudes Pendientes</p>
          <h3 className="mt-2 text-[36px] font-bold text-on-surface">{data?.metrics.pendingRequests ?? 0}</h3>
          <p className="text-[12px] text-error">Requieren revision prioritaria</p>
        </article>
        <article className="rounded-xl border border-outline-variant bg-surface-container-lowest p-md shadow-sm">
          <p className="font-label-sm uppercase tracking-wider text-on-surface-variant">Residuos Hoy (Ton)</p>
          <h3 className="mt-2 text-[36px] font-bold text-on-surface">{(data?.metrics.totalWasteTon ?? 0).toFixed(1)}</h3>
      ...
      <section className="mb-lg overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-sm">
        <div className="flex items-center justify-between border-b border-outline-variant bg-surface-container-low px-md py-4">
          <h3 className="font-headline-lg-mobile text-on-surface">Gestion de Registros</h3>
          <div className="flex gap-2">
            <button type="button" className="rounded-lg border border-outline-variant px-3 py-1.5 text-label-sm hover:bg-surface-variant">Exportar CSV</button>
            <button type="button" className="rounded-lg border border-outline-variant px-3 py-1.5 text-label-sm hover:bg-surface-variant">Filtrar</button>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-outline-variant">
              {(data?.users ?? []).map((row) => (
                <tr key={row.id} className="transition-colors hover:bg-surface-container-low">
                  <td className="px-md py-4 font-bold text-on-surface">{row.empresa}</td>
                  <td className="px-md py-4 text-on-surface-variant">{row.rol}</td>
                  ...
                  <td className="px-md py-4 text-outline">{formatDate(row.registro)}</td>
                  <td className="px-md py-4 text-right">
                    <button type="button" className="text-primary hover:underline">Administrar</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <section className="grid grid-cols-1 gap-gutter lg:grid-cols-3">
        <article className="rounded-xl border border-outline-variant bg-surface-container-lowest p-md shadow-sm lg:col-span-2">
          <div className="mb-md flex items-center justify-between">
            <h3 className="font-headline-lg-mobile text-on-surface">Volumen por Tipo de Residuo</h3>
            <span className="text-label-sm text-on-surface-variant">Top categorias</span>
          </div>
            ...
            {(data?.wasteTrend ?? []).map((item) => {
              ...
              return (
                <div key={item.type} className="flex flex-1 flex-col items-center gap-2">
                  ...
                  <span className="text-center text-[11px] text-outline">{item.type}</span>
                </div>
              );
            })}

            {!loading && (data?.wasteTrend ?? []).length === 0 && (
              <div className="w-full text-center text-on-surface-variant">Sin datos de volumen por categoria</div>
            )}
          </div>
        </article>

        <article className="rounded-xl border border-outline-variant bg-surface-container-lowest p-md shadow-sm">
          <h3 className="mb-md font-headline-lg-mobile text-on-surface">Informes Directos</h3>
