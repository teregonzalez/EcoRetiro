...
export default function PymeWasteEntryForm({ userId, username, onLogout }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (
      ...
      navigate("/dashboard/pyme/residuos/error", {
        state: {
          errorMessage:
            "Debes ingresar un tipo de residuo y una cantidad válida para continuar.",
        },
      });
      return;
    }
    ...
    try {
      ...
      navigate("/dashboard/pyme/residuos/exito", {
        state: {
          ...
          availableDate,
          notes,
          trackingId: `RET-${new Date().getFullYear()}-${Math.floor(
            100 + Math.random() * 900
          )}`,
      ...
      navigate("/dashboard/pyme/residuos/error", {
        state: {
          errorMessage:
            ...
            "Hubo una interrupción inesperada durante la validación de los datos.",
        },
      });
    } finally {
      setSubmitting(false);
  ...
  return (
    <DashboardShell
      ...
      subtitle="Logística de generación y trazabilidad de residuos"
      username={username}
