...
export default function RecyclerDashboard({
  ...
  useEffect(() => {
    const tabFromState = location.state?.recyclerTab;
    if (
    ...
    >
      {activeTab === "Residuos" ? (
        ...
        <>
          ...
          <section className="mb-lg grid grid-cols-1 gap-gutter md:grid-cols-4">
            {metricCards.map((metric) => (
              ...
              >
                ...
                <span className={`material-symbols-outlined text-4xl ${metric.color}`}>
                  {metric.icon}
                </span>
              </article>
            ))}
          ...
          {/* Mapa de Residuos Cercanos y Capacidad de Planta */}
          <section className="mb-lg grid grid-cols-1 gap-gutter lg:grid-cols-12">
            
            {/* Mapa Interactivo / Residuos Cercanos */}
                  ...
                  {(data?.nearbyWaste ?? []).slice(0, 4).map((item, idx) => (
                    <li key={item.material}>
                      <button
                        ...
                        className={`w-full rounded-lg bg-surface-container p-2 text-left transition-colors hover:bg-surface-container-high ${
                          idx === 0
                ...
                <tbody className="divide-y divide-outline-variant">
                  ...
                  {!loading && (data?.collectionHistory ?? []).length === 0 && (
                    <tr>
                      ...
                      >
                        No hay historial de recolección disponible
                      </td>
                    </tr>
                  )}
                </tbody>
